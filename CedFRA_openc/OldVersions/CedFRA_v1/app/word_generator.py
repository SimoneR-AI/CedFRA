...
def _sostituisci_testo_run(paragraph, placeholder, nuovo_valore):
    ...
    new_text = full_text.replace(placeholder, nuovo_valore)

    # Assegna il nuovo testo al primo run, svuota gli altri
    if paragraph.runs:
...
def costruisci_nome_file_pagamento(cognome: str, mese: int, anno: int) -> str:
    ...
    mese_abbr = CONFIG["mesi_abbreviati"][mese - 1]
...
def costruisci_nome_file_mese(cognome: str, mese: int, anno: int) -> str:
    ...
    """
    mese_upper = CONFIG["mesi_italiano_upper"][mese - 1]
