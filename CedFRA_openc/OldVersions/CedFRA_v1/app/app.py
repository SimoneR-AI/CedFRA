...
v1.0.1 — Sviluppata da Simone Robbiani
"""
...
class CedFRAApp:
    def __init__(self, root):
        ...
        # Icona finestra (usa il logo se disponibile)
    ...
    def _make_card(self, parent, **grid_kw):
        """Crea un frame 'card' con sfondo bianco e bordo."""
    ...
    def _section_title(self, parent, text, row):
        """Etichetta titolo sezione con linea colorata."""
    ...
    def _crea_interfaccia(self):
        ...
        # Barra decorativa superiore (richiama carta intestata)
        top_accent = tk.Frame(header, bg=BRAND_GOLD, height=3)
        top_accent.grid(row=0, column=0, columnspan=3, sticky="ew")

        # Logo (versione bianca per header scuro)
        logo_path = os.path.join(ASSETS_DIR, "logo_header.png")
        if os.path.exists(logo_path):
            try:
                self._logo_img = tk.PhotoImage(file=logo_path)
                logo_lbl = tk.Label(header, image=self._logo_img, bg=BRAND_NAVY,
                                    borderwidth=0)
                logo_lbl.grid(row=1, column=0, padx=(20, 16), pady=12, sticky="w")
            except tk.TclError:
                self._logo_img = None
                tk.Label(header, text="MARINE INTERIORS",
                         font=("Segoe UI Black", 14), bg=BRAND_NAVY,
                         fg=COLOR_WHITE).grid(row=1, column=0, padx=(20, 16),
                                              pady=12, sticky="w")
        else:
            tk.Label(header, text="MARINE INTERIORS",
                     font=("Segoe UI Black", 14), bg=BRAND_NAVY,
        ...
        self.btn_carica = tk.Button(
            btn_frame, text="  Carica PDF cedolino  ",
            font=("Segoe UI Semibold", 11), bg=BRAND_NAVY, fg=COLOR_WHITE,
            activebackground=BRAND_NAVY_LIGHT, activeforeground=COLOR_WHITE,
            relief="flat", cursor="hand2", padx=20, pady=10,
            command=self._carica_pdf,
        )
        self.btn_carica.grid(row=0, column=0, sticky="ew")

        # ── Card: Dati dal cedolino ──
        card1 = self._make_card(body, row=1, column=0, sticky="ew", pady=(0, 10))
        ...
        card2 = self._make_card(body, row=2, column=0, sticky="ew", pady=(0, 10))
        ...
        # ── Card: Data pagamento ──
        card3 = self._make_card(body, row=3, column=0, sticky="ew", pady=(0, 10))
        ...
        self.btn_genera.grid(row=0, column=0, sticky="ew", padx=(0, 6))

        btn_apri = tk.Button(
            btns, text="Apri cartella output",
        ...
        btn_apri.grid(row=0, column=1, sticky="ew", padx=(6, 0))

        # ── Card: Log ──
        card_log = self._make_card(body, row=5, column=0, sticky="ew", pady=(0, 6))
        ...
        version_lbl = tk.Label(footer, text=f"v{APP_VERSION} (Changelog)",
        ...
        version_lbl.bind("<Button-1>", lambda e: self._mostra_changelog())
    ...
    def _log(self, messaggio: str):
        """Aggiunge un messaggio all'area log con timestamp."""
    ...
    def _carica_pdf(self):
        """Apre un file dialog per selezionare il PDF del cedolino."""
        ...
        if cognome not in CONFIG["dipendenti"]:
            messagebox.showerror(
                "Errore",
                f"Dipendente '{cognome}' non trovato nella configurazione.\n"
                f"Dipendenti configurati: {', '.join(CONFIG['dipendenti'].keys())}",
            )
            self._log(f"ERRORE: Dipendente '{cognome}' non configurato.")
            return

        self.dati_pdf = dati
        self.dati_dipendente = CONFIG["dipendenti"][cognome]

        # Aggiorna campi GUI
        nome_completo = f"{dati['cognome']} {dati['nome']}"
        mese_nome = CONFIG["mesi_italiano"][dati["mese"] - 1].capitalize()
    ...
    def _valida_input(self) -> bool:
        """Valida i campi input dell'utente."""
    ...
    def _genera_documenti(self):
        """Genera i due documenti Word per il dipendente selezionato."""
        ...
        # Data pagamento (override o calcolata)
        data_override = self.entry_data_override.get().strip()
        if data_override:
        ...
        # Nomi mesi
        mese_italiano = CONFIG["mesi_italiano"][mese - 1]
        mese_francese = CONFIG["mesi_francese"][mese - 1]

        # Assicurati che la cartella output esista
        os.makedirs(OUTPUT_DIR, exist_ok=True)

        # --- 1. Documento Pagamento ---
        dati_pag = {
            "prot_num": prot_num,
            "prot_data": prot_data,
        ...
        path_pag = os.path.join(OUTPUT_DIR, nome_pag)
        template_pag = os.path.join(TEMPLATES_DIR, "template_pagamento.docx")

        genera_pagamento(template_pag, path_pag, dati_pag)
        self._log(f"Generato: {nome_pag}")

        # --- 2. Documento Mese Anno ---
        causale = (f"{CONFIG['societa_completa']} \u2013 "
        ...
        path_mese = os.path.join(OUTPUT_DIR, nome_mese)
        template_mese = os.path.join(TEMPLATES_DIR, "template_mese.docx")
    ...
    def _apri_cartella(self):
        """Apre la cartella output in Esplora risorse."""
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        subprocess.Popen(["explorer", os.path.normpath(OUTPUT_DIR)])

    def _mostra_changelog(self):
        """Mostra il contenuto del changelog."""
        changelog_path = os.path.join(APP_DIR, "CHANGELOG.md")
        if os.path.exists(changelog_path):
            with open(changelog_path, "r", encoding="utf-8") as f:
                testo = f.read()
            messagebox.showinfo(f"Changelog v{APP_VERSION}", testo)
        else:
            messagebox.showinfo(f"Changelog v{APP_VERSION}", "File CHANGELOG.md non trovato.")
