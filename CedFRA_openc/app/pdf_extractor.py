...
def estrai_dati_cedolino(pdf_path: str) -> dict:
    ...
    # --- Header: MIS.P.A##BULLETIN##MM-YYYY##MATRICOLA##COGNOME##Nome##SIRET
    # Cerca in tutto il testo per robustezza (il PDF potrebbe avere righe vuote iniziali)
    header_match = None
    for line in lines:
        m = re.match(
    ...
    if not header_match:
        # Fallback: cerca con search se la riga ha spazi o prefissi
        m = re.search(
            r"MIS\.P\.A##BULLETIN##(\d{2})-(\d{4})##\d+##(\w+)##(\w+)##",
            text,
        )
        if m:
            ...
            raise ValueError(
                f"Header del cedolino non riconosciuto nel PDF: {pdf_path}"
            )
    ...
    # --- Net payé: cerca la riga con l'importo netto
    net_paye = None
    for line in lines:
